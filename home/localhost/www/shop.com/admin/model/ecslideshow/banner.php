...
class ModelEcslideshowBanner extends Model {
	...
	public function upgradeToLatest(){
		...
		$query = $this->db->query("SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = '".DB_DATABASE."' AND TABLE_NAME = '".DB_PREFIX."ecbanner_image' AND COLUMN_NAME = 'product_id'");
		if($query->num_rows <= 0){
			$query = $this->db->query("ALTER TABLE `".DB_DATABASE."`.`".DB_PREFIX."ecbanner_image` ADD COLUMN `product_id` int(11) DEFAULT NULL ");
		}
	}
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			$banner_image_data[] = array(
        		...
        		'product_id'               => isset($banner_image['product_id'])?$banner_image['product_id']:0,
